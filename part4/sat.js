...
function papadimitrou(filename){
    ...
    outerloop: while(counter1<counter1Max){
        let values= Array(n).fill().map(()=>Math.random()>=0.5? true:false);
        let check=true;
        ...
        while(counter2<counter2Max){
            ...
            values[Math.abs(lines[unsatisfiedIndex][randomIndex])]=-values[Math.abs(lines[unsatisfiedIndex][randomIndex])];
            counter2++;
        }
        counter1++;
    }
